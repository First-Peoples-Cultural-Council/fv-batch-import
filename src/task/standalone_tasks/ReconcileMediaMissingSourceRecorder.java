...
public class ReconcileMediaMissingSourceRecorder {
	...
	public ReconcileMediaMissingSourceRecorder(CommandLine cmd) throws Exception {
        ...
        try {
            ...
            for (int j = 0; j < 1; j++) {
                String sql = ""
                        + "SELECT * FROM FVAudio, FVVideo, FVPicture "
                        + "WHERE "
                        + "ecm:path STARTSWITH '/FV/sections' "
                        + "AND "
                        + "((fvm:source/* IS NOT NULL AND fvproxy:proxied_source/* IS NULL) "
                        + "OR (fvm:recorder/* IS NOT NULL AND fvproxy:proxied_recorder/* IS NULL)) "
                        + "AND dc:modified NOT BETWEEN DATE '2017-08-18' AND DATE '2017-08-30'";

                Documents docs = (Documents) session.newRequest("Repository.Query").setHeader(
                        Constants.HEADER_NX_SCHEMAS, "*")
                        .set("query", sql)
                        .set("currentPageIndex", 0).set("pageSize", 1).set("sortBy","dc:modified").set("sortOrder", "desc").execute();
...
//                                }
	...
	public static void main(String[] argv) throws Exception {
		...
		new ReconcileMediaMissingSourceRecorder(cmd);
	}
}
