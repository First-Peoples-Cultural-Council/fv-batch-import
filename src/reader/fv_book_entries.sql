SELECT
  SENTRY.ID,
  SENTRY.SENTRY_BOOK_ID,  
  SENTRY.DOMINANT_LANGUAGE_TEXT,
  SENTRY.DOMINANT_LANGUAGE_TRANSLATION,
  SENTRY.ABORIGINAL_LANGUAGE_TEXT,
  SENTRY.ASSIGNED_USR_ID,
  SENTRY.CULTURAL_NOTE,
  SENTRY.IMAGE_ENTRY_ID,
  SENTRY.IMAGE_STATUS_ID,
  SENTRY.SOUND_ENTRY_ID,
  SENTRY.SOUND_STATUS_ID,
  SENTRY.VIDEO_ENTRY_ID,
  SENTRY.VIDEO_STATUS_ID,  
  SENTRY.CHANGE_DTTM,
  SENTRY.SORT_MAP,
  SENTRY.ASSIGNED_USR_ID,
...
FROM FIRSTVOX.SENTRY

LEFT JOIN FIRSTVOX.SENTRY_BOOK ON (SENTRY.SENTRY_BOOK_ID = SENTRY_BOOK.ID)
...
LEFT JOIN FIRSTVOX.ENTRY_MEDIA ENTRY_IMAGE ON (ENTRY_IMAGE.ID = SENTRY.IMAGE_ENTRY_ID)
LEFT JOIN FIRSTVOX.ENTRY_MEDIA ENTRY_VIDEO ON (ENTRY_VIDEO.ID = SENTRY.VIDEO_ENTRY_ID)
LEFT JOIN FIRSTVOX.ENTRY_MEDIA ENTRY_AUDIO ON (ENTRY_AUDIO.ID = SENTRY.SOUND_ENTRY_ID)

ORDER BY SENTRY_BOOK_ID, SENTRY.SORT_MAP;