...
public class FVCategoryMigrator extends AbstractMigrator {
	...
	protected Map<String, Document> getOrCreateSharedCategoriesFolder(NuxeoClient client, AbstractReader reader)
			throws IOException {
		...
		try {			
			...
			sharedCategoriesFolder = client.schemas("*").repository().fetchDocumentByPath(sharedDataPath + sharedCategoriesFolderName);
			
			// FV/Sections/SharedData/Shared Categories
			sectionSharedCategoriesFolder = client.schemas("*").repository().fetchDocumentByPath(sectionSharedDataPath + sharedCategoriesFolderName);
			...
			sharedCategoriesFolder = client.operation("Document.Create").schemas("*")
					.input(root).param("type", sharedCategoriesFolder.getType()).param("name", sharedCategoriesFolder.getId())
					.param("properties", sharedCategoriesFolder).execute();
			
			// Publish Shared Categories folder to the section
			sectionSharedCategoriesFolder = client.operation("Document.Publish").input(sharedCategoriesFolder).param("target", sectionSharedDataPath).execute();
		}		
	...
	protected Map<String, Document> getParentCategory(String parentCategoryId, NuxeoClient client, AbstractReader reader)
			throws IOException {
		...
		try {			
			parentCategory = client.schemas("*").repository().fetchDocumentByPath(sharedDataPath + sharedCategoriesFolderName + "/" + parentCategoryId);
		} catch (NuxeoClientException e) {
			// Should use exists first but prefer to reduce number of network calls
			if (!e.getCause().getMessage().contains("DocumentNotFoundException")) {
				throw e;
		...
		try {
			sectionParentCategory = client.schemas("*").repository().fetchDocumentByPath(sectionSharedDataPath + sharedCategoriesFolderName + "/" + parentCategoryId);
		} catch (NuxeoClientException e) {
			// Should use exists first but prefer to reduce number of network calls
			if (!e.getCause().getMessage().contains("DocumentNotFoundException")) {
				throw e;
