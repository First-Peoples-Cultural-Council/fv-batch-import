...
public class FirstVoicesMigrator extends AbstractMigrator {
    ...
    protected void processRow(NuxeoClient client) throws IOException {
		...
		setProperty(word, "fv:source", processWordSources(reader.getString("CONTRIBUTER"), client, docs.get("Contributors")));
		...
		method_start = System.currentTimeMillis();
		setProperty(word, "fv:related_pictures", processWordImages(reader, client, docs.get("Resources"), docs.get("Contributors")));
		System.out.println("ProcessImage: " + Long.valueOf(System.currentTimeMillis() - method_start));
		method_start = System.currentTimeMillis();
		setProperty(word, "fv:related_audio", processWordAudio(reader, client, docs.get("Resources"), docs.get("Contributors")));
		System.out.println("ProcessAudio: " + Long.valueOf(System.currentTimeMillis() - method_start));
		method_start = System.currentTimeMillis();
		setProperty(word, "fv:related_videos", processWordVideo(reader, client, docs.get("Resources"), docs.get("Contributors")));
		System.out.println("ProcessVideo: " + Long.valueOf(System.currentTimeMillis() - method_start));

		setProperty(word, "fv-word:related_phrases", processWordSamplePhrase(phraseTitle, phraseTranslation, dominantLanguage, client, docs.get("Dictionary")));
	...
	public boolean checkWordCreated(String wordId, NuxeoClient client, Document dictionary) throws IOException {
     ...
     */
    public String processWordSources(String migratedSources, NuxeoClient client, Document contributorsFolder) {
            String sourcesList = "";
                    ...
                    for (int i = 0; i < sourcesArray.length; i++) {
                            ...
                            try {
                                    ...
                                    if (existingContributorDocs.getDocuments().isEmpty()) {
                                            ...
                                            Document newContributorDocument = Document.createWithName(source.replace("/", "_"), "FVContributor");
                                            newContributorDocument.setPropertyValue("dc:title", source);
                                            contributor = (Document) client.operation("Document.Create")
                                                            .schemas("*").input(contributorsFolder)
                                                            .param("type", newContributorDocument.getType())
                                                            .param("name", newContributorDocument.getId()).param("properties", newContributorDocument)
                                                            .execute();
                                    }
	...
	protected String processWordAudio(AbstractReader reader, NuxeoClient client, Document resourcesFolder, Document contributorsFolder)
			throws IOException {
		return processWordBinary(reader, "AUDIO", client, resourcesFolder, contributorsFolder, "FVAudio");
	}

	protected String processWordVideo(AbstractReader reader, NuxeoClient client, Document resourcesFolder, Document contributorsFolder)
			throws IOException {
		return processWordBinary(reader, "VIDEO", client, resourcesFolder, contributorsFolder, "FVVideo");
	}

	protected String processWordImages(AbstractReader reader, NuxeoClient client, Document resourcesFolder, Document contributorsFolder)
			throws IOException {
		return processWordBinary(reader, "IMG", client, resourcesFolder, contributorsFolder, "FVPicture");
	}

	protected String processWordBinary(AbstractReader reader, String prefix, NuxeoClient client, Document resourcesFolder,
		...
		binaryDoc.setPropertyValue("fvm:source", processWordSources(binaryContributor, client, contributorsFolder));
		binaryDoc.setPropertyValue("fvm:recorder", processWordSources(binaryRecorder, client, contributorsFolder));
    ...
    public String processWordSamplePhrase(String phraseTitle, String phraseTranslation, String dominantLanguage, NuxeoClient client, Document dictionaryFolder) {
    	String uid = null;
        ...
        try {
            ...
            if (existingPhraseDocs.getDocuments().isEmpty()) {
            	...
            	phrase = (Document) client.operation("Document.Create")
